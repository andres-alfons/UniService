...
export default function ModalReporte({ onClose, idServicio = null, idSolicitud = null }) {
  ...
  const handleImagenes = (e) => {
    ...
    if (imagenes.length + nuevas.length > 3) {
      setError("Máximo 3 imágenes de evidencia.");
      return;
    }
    setError("");
    ...
    setPreviews((prev) => [
      ...prev,
      ...nuevas.map((f) => ({ preview: URL.createObjectURL(f) })),
    ]);
  ...
  const handleEnviar = async () => {
    ...
    try {
      // 1. Crear el reporte (sin imágenes aún)
      ...
      if (!res.ok || !data.ok) {
        setError(data.error || "No se pudo crear el reporte.");
        return;
      }

      const idReporte = data.id_reporte;

      // 2. Si hay imágenes, subirlas al endpoint dedicado
      if (imagenes.length > 0) {
        const formData = new FormData();
        imagenes.forEach((img) => formData.append("imagenes", img));

        const imgRes = await fetch(`/api/reportes/${idReporte}/imagenes`, {
          method: "POST",
          body: formData,
        });
        ...
        if (!imgRes.ok) {
          ...
          // El reporte ya se creó, solo avisamos del fallo de imágenes
          setError(`Reporte enviado, pero falló la subida de imágenes: ${imgData.error}`);
          setExito(true);
          return;
        }
      }
  ...
  const lbl = {
    fontSize: "0.82rem", color: "var(--texto2, #aaa)",
        ...
        {exito ? (
          <div style={{ textAlign: "center", padding: "20px 0" }}>
            ...
            <p style={{ color: "var(--texto2, #aaa)", fontSize: "0.9rem" }}>
              Nuestro equipo revisará tu reporte a la brevedad. Puedes ver el
              estado en tu perfil, sección "Mis reportes".
            </p>
          ...
          <>
            ...
            <input
              ...
              value={form.titulo} onChange={handleChange} style={inp}
            />
            ...
            {/* ── SECCIÓN DE IMÁGENES ── */}
            <label style={{ ...lbl, marginTop: "18px" }}>
              Imágenes de evidencia (máx. 3) — opcional
            </label>
            ...
            {imagenes.length < 3 && (
              <button
                ...
                style={{
                  ...
                  border: "2px dashed var(--borde, #444)",
                  background: "var(--fondo, #12121f)",
                  color: "var(--texto2, #aaa)", cursor: "pointer",
                  fontSize: "0.9rem", transition: "all 0.2s",
                }}
                onMouseEnter={(e) => {
                  e.currentTarget.style.borderColor = "var(--teal, #00b4d8)";
                  e.currentTarget.style.color = "var(--teal, #00b4d8)";
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.borderColor = "var(--borde, #444)";
                  e.currentTarget.style.color = "var(--texto2, #aaa)";
                }}
              >
              ...
              <div style={{
                ...
                {previews.map((p, i) => (
                  <div key={i} style={{
                    ...
                    border: "2px solid var(--borde, #444)",
                  }}>
                    <img
                      src={p.preview} alt={`evidencia ${i + 1}`}
            ...
            <div style={{ display: "flex", gap: "10px", justifyContent: "flex-end", marginTop: "20px" }}>
              ...
              <button style={btnPrimary} onClick={handleEnviar} disabled={enviando}>
                {enviando
                  ? imagenes.length > 0 ? "Subiendo imágenes..." : "Enviando..."
                  : "Enviar reporte"}
              </button>
            </div>
          </>
